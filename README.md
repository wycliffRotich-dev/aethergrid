...
- **Standalone worker agent**: `scripts/run_agent.py` runs as a real, separate process, polling the API over HTTP for assigned work, executing it as a real local subprocess, and heartbeating on its own background thread for the agent's entire lifetime, independent of whatever job it's currently executing (see [ADR 0019](docs/adr/0019-standalone-worker-agent-process.md)). This replaces the dashboard's client-side heartbeat as the liveness mechanism for any worker running it; a worker with no agent process attached still falls back to node liveness alone. The agent does not yet have exclusive ownership over its worker's job execution: the in-process scheduler loop still schedules and executes jobs on its own one-second tick regardless of whether a standalone agent is also polling, so a job is currently completed by whichever path reaches it first. Giving the agent real exclusive ownership is the next piece of ADR 0019's staged plan, not yet built.
...
- **Explicit execution-start confirmation**: `POST /workers/{worker_id}/jobs/{job_id}/start` lets whatever is actually executing a job, today the same process, eventually a standalone agent per ADR 0019, confirm execution has genuinely begun. This is the one call that transitions a job from `Scheduled` to `Running`; assignment alone no longer does (see [ADR 0019](docs/adr/0019-standalone-worker-agent-process.md))
...
257 tests across domain, application, infrastructure, and API layers, 254 passing and 3 deliberately skipped, not silently ignored:
...
- Exclusive job ownership for the standalone worker agent: the agent (see above) can register, heartbeat, and poll for work independently of the dashboard, but has no way yet to claim exclusive execution rights over a job, since the in-process scheduler loop still races it for every assigned job and typically wins. This is the actual remaining piece of ADR 0019's staged cutover, not the agent's existence
